...
--------------------")
 ...
 command = "fswebcam -D 3 -S 10 --set brightness=50% --set contrast=20% 
-F 5 -r 1280x720 --no-banner " + file_location
 os.system(command)
...
def on_chat_message(msg):
 ...
 bot.sendMessage(chat_id,text="Invalid date format please send
again !\n")
 ...
 message += "1. /getdata : To retrieve the images capture on a
day or a given duration of time."
 bot.sendMessage(chat_id,message)
 elif req == "/getdata":
 #date = findDate()
 message = "Data on your storage is available from the date : "
#+ date + "\n" 
 message += "Select an option :"
 keyboard = InlineKeyboardMarkup(inline_keyboard=[
 [InlineKeyboardButton(text='1. Get Data of a given
date.', callback_data='singleDate')],
 [InlineKeyboardButton(text='2. Get Data using start
and end date.', callback_data='multipleDate')]
 ])
...
def sendData(data1,data2):
 ...
 message = "Date : " + folder[6:8] + "-" + folder[4:6] + "-" +
folder[0:4] + "\n"
 message += "Found [" + str(len(files)) + "] results."
 bot.sendMessage(chat_id,message)
 for f in files:
 message2 = "Time - " + f[8:10] + " : " + f[10:12] + " : "
+ f[12:14]
...
def main():
 ...
 MessageLoop(bot,{'chat': on_chat_message,'callback_query':
on_callback}).run_as_thread()
 ...
 snap()
 thread1.join()
 #sleep is important so that Pi does not get damaged due to
infinite loop
 sleep(0.1)
...
# calling main function from here to prevent creating inifinite threads of
infinite loops 
